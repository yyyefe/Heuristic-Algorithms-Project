...
def GWO(SearchAgents_no, Max_iter, param_grid, Xtrain_scaled, ytrain, Xtest_scaled, ytest):
    ...
    for l in range(0, Max_iter):
        ...
        for i in range(0, SearchAgents_no):
            ...
            D_alpha = abs(C1 * Alpha_pos['n_estimators'] - Positions[i]['n_estimators'])
            # Equation (3.5)-part 1
            X1_1 = Alpha_pos['n_estimators'] - A1 * D_alpha
            # Equation (3.6)-part 1
            D_alpha = abs(C1 * Alpha_pos['learning_rate'] - Positions[i]['learning_rate'])
            # Equation (3.5)-part 1
            X1_2 = Alpha_pos['learning_rate'] - A1 * D_alpha
            # Equation (3.6)-part 1
            D_alpha = abs(C1 * Alpha_pos['max_depth'] - Positions[i]['max_depth'])
            # Equation (3.5)-part 1
            X1_3 = Alpha_pos['max_depth'] - A1 * D_alpha
            # Equation (3.6)-part 1
            D_alpha = abs(C1 * Alpha_pos['min_samples_split'] - Positions[i]['min_samples_split'])
            # Equation (3.5)-part 1
            X1_4 = Alpha_pos['min_samples_split'] - A1 * D_alpha
            # Equation (3.6)-part 1
            D_alpha = abs(C1 * Alpha_pos['min_samples_leaf'] - Positions[i]['min_samples_leaf'])
            # Equation (3.5)-part 1
            X1_5 = Alpha_pos['min_samples_leaf'] - A1 * D_alpha
            # Equation (3.6)-part 1
            ...
            D_beta = abs(C2 * Beta_pos['n_estimators'] - Positions[i]['n_estimators'])
            # Equation (3.5)-part 2
            X2_1 = Beta_pos['n_estimators'] - A2 * D_beta
            # Equation (3.6)-part 2
            D_beta = abs(C2 * Beta_pos['learning_rate'] - Positions[i]['learning_rate'])
            # Equation (3.5)-part 2
            X2_2 = Beta_pos['learning_rate'] - A2 * D_beta
            # Equation (3.6)-part 2
            D_beta = abs(C2 * Beta_pos['max_depth'] - Positions[i]['max_depth'])
            # Equation (3.5)-part 2
            X2_3 = Beta_pos['max_depth'] - A2 * D_beta
            # Equation (3.6)-part 2
            D_beta = abs(C2 * Beta_pos['min_samples_split'] - Positions[i]['min_samples_split'])
            # Equation (3.5)-part 2
            X2_4 = Beta_pos['min_samples_split'] - A2 * D_beta
            # Equation (3.6)-part 2
            D_beta = abs(C2 * Beta_pos['min_samples_leaf'] - Positions[i]['min_samples_leaf'])
            # Equation (3.5)-part 2
            X2_5 = Beta_pos['min_samples_leaf'] - A2 * D_beta
            # Equation (3.6)-part 2
            ...
            D_delta = abs(C3 * Delta_pos['n_estimators'] - Positions[i]['n_estimators'])
            # Equation (3.5)-part 3
            X3_1 = Delta_pos['n_estimators'] - A3 * D_delta
            # Equation (3.5)-part 3
            D_delta = abs(C3 * Delta_pos['learning_rate'] - Positions[i]['learning_rate'])
            # Equation (3.5)-part 3
            X3_2 = Delta_pos['learning_rate'] - A3 * D_delta
            # Equation (3.5)-part 3
            D_delta = abs(C3 * Delta_pos['max_depth'] - Positions[i]['max_depth'])
            # Equation (3.5)-part 3
            X3_3 = Delta_pos['max_depth'] - A3 * D_delta
            # Equation (3.5)-part 3
            D_delta = abs(C3 * Delta_pos['min_samples_split'] - Positions[i]['min_samples_split'])
            # Equation (3.5)-part 3
            X3_4 = Delta_pos['min_samples_split'] - A3 * D_delta
            # Equation (3.5)-part 3
            D_delta = abs(C3 * Delta_pos['min_samples_leaf'] - Positions[i]['min_samples_leaf'])
            # Equation (3.5)-part 3
            X3_5 = Delta_pos['min_samples_leaf'] - A3 * D_delta
            # Equation (3.5)-part 3

            if (X1_1 + X2_1 + X3_1) / 3 > 1:
                Positions[i]['n_estimators'] = int((X1_1 + X2_1 + X3_1) / 3)  # Equation (3.7)
            if (X1_2 + X2_2 + X3_2) / 3 > 0:
                Positions[i]['learning_rate'] = (X1_2 + X2_2 + X3_2) / 3  # Equation (3.7)
            if (X1_3 + X2_3 + X3_3) / 3 > 1:
                Positions[i]['max_depth'] = int((X1_3 + X2_3 + X3_3) / 3)  # Equation (3.7)
            if ((X1_4 + X2_4 + X3_4) / 3) > 2:
                Positions[i]['min_samples_split'] = int((X1_4 + X2_4 + X3_4) / 3)  # Equation (3.7)
            if (X1_5 + X2_5 + X3_5) / 3 > 1:
                Positions[i]['min_samples_leaf'] = int((X1_5 + X2_5 + X3_5) / 3)  # Equation (3.7)
